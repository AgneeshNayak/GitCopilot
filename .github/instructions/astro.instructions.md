...
- Define and document a `Props` interface in the frontmatter of every reusable component. The documentation should describe the component contract and any non-obvious prop constraints.
- For page-only props, a local `Props` interface is still preferred when it improves clarity; do not duplicate a component's contract in prose elsewhere.
- Type component imports and helper return values
- Comment intent and non-obvious rendering decisions, not markup that is already self-explanatory. Keep comments current when the component changes.
